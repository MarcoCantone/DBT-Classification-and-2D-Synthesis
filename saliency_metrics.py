...
def save_surfaces(dataset, config):
    ...
    for k, (img, label, img_path, bbox, _) in enumerate(dataset):
        ...
        # fig, ax = plt.subplots()
        # ax.imshow(img[bbox[0]], cmap='gray', origin='upper')  # or use 'lower' to match coord
        #
        ...
        output_image = img[img.shape[0]//2]
        ...
        # fig, ax = plt.subplots()
        # ax.imshow(output_image, cmap='gray', origin='upper')  # or use 'lower' to match coord
        #
        # rect = patches.Rectangle(
        #     (bbox[1], bbox[2]), bbox[3], bbox[4],
        #     linewidth=2, edgecolor='r', facecolor='none'
        ...
        # print("ssim score list:")
        # print(ssim_list_score)
        # print("ssim diff list:")
        # print(ssim_list_diff)
        # print("distances from Z:")
        # print(distances)
        # print("LPIPS Score list:")
        # print(lpips_list_score)
        print(k)
    ...
    best_value = max(ssim_list_score)
    best_index = ssim_list_score.index(best_value)

    img, label, img_path, bbox, _ = dataset[max_index]
    ...
    img_original = (img[bbox[0]].numpy() * 255).astype(np.uint8)
    img_original = cv2.cvtColor(img_original, cv2.COLOR_GRAY2BGR)

    cv2.rectangle(img_original, (bbox[1], bbox[2]), (bbox[1] + bbox[3], bbox[2] + bbox[4]), color=(0, 0, 255), thickness=4)

    cv2.imwrite('/ssd2/dellascenza/datasets/BCS-DBT_cropped_boxes/fuorifuoco_originale.png', img_original)

    prediction, attn_weights = model(img[None, :, :, :], True)

    print("PREDICTION FUORI FUOCO:")
    print(prediction)
    ...
    #     "/ssd2/dellascenza/datasets/BCS-DBT_cropped_boxes/fuorifuoco.png")

    img_synt = (output_image * 255).astype(np.uint8)
    img_synt = cv2.cvtColor(img_synt, cv2.COLOR_GRAY2BGR)

    cv2.rectangle(img_synt, (bbox[1], bbox[2]), (bbox[1] + bbox[3], bbox[2] + bbox[4]), color=(0, 0, 255), thickness=4)

    cv2.imwrite('/ssd2/dellascenza/datasets/BCS-DBT_cropped_boxes/fuorifuoco_synt.png', img_synt)

    #---------------------
    print("WORSE DISTANCE:" + str(max_value))
    print("BEST SSIM:" + str(best_value))
    img, label, img_path, bbox, _ = dataset[best_index]
    ...
    # Image.fromarray(img[bbox[0]].numpy() * ((2 ** 8) - 1)).convert("RGB").save(
    #     "/ssd2/dellascenza/datasets/BCS-DBT_cropped_boxes/fuorifuoco_originale.png")
    ...
    cv2.imwrite('/ssd2/dellascenza/datasets/BCS-DBT_cropped_boxes/bestssim_originale.png', img_original)

    prediction, attn_weights = model(img[None, :, :, :], True)

    print("PREDICTION BEST SSIM:")
    print(prediction)
    ...
    cv2.imwrite('/ssd2/dellascenza/datasets/BCS-DBT_cropped_boxes/bestssim_synt.png', img_synt)
